    ...
    <div class="flex min-h-screen" x-data="{ sidebarOpen: false }">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                <a href="/empresa/dashboard" @click="sidebarOpen = false"
                    class="block px-4 py-3 rounded-xl transition {{ request()->routeIs('empresa.dashboard') ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 text-white' }}">
                    Dashboard
                </a>
                <a href="/empresa/perfil" @click="sidebarOpen = false"
                    class="block px-4 py-3 rounded-xl transition {{ request()->routeIs('empresa.perfil') ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 text-white' }}">
                    Perfil Empresa
                </a>
                <a href="/empresa/talentos" @click="sidebarOpen = false"
                ...
                <a href="/empresa/usuarios" @click="sidebarOpen = false"
                    class="block px-4 py-3 rounded-xl transition {{ request()->routeIs('empresa.usuarios') ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 text-white' }}">
                    Usuarios Asociados
                </a>
                <a href="/empresa/documentos" @click="sidebarOpen = false"
                    class="block px-4 py-3 rounded-xl transition {{ request()->routeIs('empresa.documentos') ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 text-white' }}">
                    Documentos
                </a>
            </nav>
        </aside>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="bg-white shadow px-4 md:px-8 py-4 flex justify-between items-center sticky top-0 z-10">
                <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-xl md:text-2xl font-bold text-gray-800">Portal Empresa</h1>
                </div>
