<x-talento-layout>
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-500 mt-2">
                    Tus cargos y posiciones laborales anteriores.
                </p>
            </div>
                ...
                <div class="flex items-start justify-between">
                    ...
                    <div class="flex-1">
                        ...
                        <div class="mb-3 flex items-center gap-2">
                            <span>
                                {{ $item->egreso ? 'Finalizada' : 'Actual' }}
                            </span>
                        </div>

                        <h2 class="text-2xl font-bold text-gray-800">
                            {{ $item->institucion_o_empresa }}
                        </h2>
                    ...
                    <div class="flex gap-2 ml-4 flex-shrink-0">
                        ...
                        <button
                            onclick="document.getElementById('editar{{ $item->id }}').classList.remove('hidden')"
                        ...
                        <form action="{{ route('talento.experiencia.destroy', $item->id) }}" method="POST"
                            class="inline" onsubmit="return confirm('¿Estás seguro de eliminar esta experiencia?');">
        ...
        @empty
            ...
            <div class="bg-white rounded-2xl shadow p-16 text-center">
                ...
                <p class="text-gray-500 mt-3 mb-8">
                    Agrega tus experiencias laborales para destacar tu perfil profesional.
                </p>
                            ...
                            <!-- FUNCIONES -->
                                ...
                                <!-- REFERENCIA LABORAL -->
                                    ...
                                    <div>
                                        ...
                                        <div class="flex">
                                            <span
                                                class="px-4 py-3 border border-r-0 border-gray-300 rounded-l-xl bg-gray-100">
                                                +569
                                            </span>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @foreach ($experiencias as $item)
                ...
                const checkbox{{ $item->id }} =
                    document.getElementById('actualidad{{ $item->id }}');

                const fecha{{ $item->id }} =
                    document.getElementById('egreso{{ $item->id }}');

                if (checkbox{{ $item->id }} && fecha{{ $item->id }}) {

                    if (checkbox{{ $item->id }}.checked) {
                        fecha{{ $item->id }}.disabled = true;
                    }
        ...
        <div class="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div class="bg-blue-600 px-8 py-6 text-white flex items-center justify-between sticky top-0">
                ...
                <div>
                    ...
                    <p class="text-blue-100 mt-1 text-sm">
                        Agrega un cargo o posición laboral.
                    </p>
                </div>
            ...
            <form action="{{ route('talento.experiencia.store') }}" method="POST" class="p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="cargo" value="{{ old('cargo') }}" required minlength="3"
                            maxlength="100"
                            class="w-full border {{ $errors->has('cargo') ? 'border-red-500 bg-red-50' : 'border-gray-300' }} rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none transition"
                            placeholder="Ej: Desarrollador Senior">
                        @error('cargo')
                    ...
                    <div>
                        ...
                        <input type="text" name="institucion_o_empresa"
                            value="{{ old('institucion_o_empresa') }}" required minlength="3" maxlength="100"
                            class="w-full border {{ $errors->has('institucion_o_empresa') ? 'border-red-500 bg-red-50' : 'border-gray-300' }} rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 outline-none transition"
                            placeholder="Ej: Acme Corp">
                        @error('institucion_o_empresa')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div class="md:col-span-2">
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <input type="checkbox" name="actualidad" id="actualidadCrear"
                                class="w-5 h-5 text-blue-600 rounded">
                            <span class="font-medium text-gray-700">
                                Actualmente trabajo aquí
                            </span>
                        ...
                        <div class="border-t pt-6 mt-2">
                            ...
                            <div class="grid md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="referencia_cargo"
                                        value="{{ old('referencia_cargo') }}"
                                        class="w-full border border-gray-300 rounded-xl px-4 py-3">
                                </div>

                                <div class="flex">
                                    <span class="px-4 py-3 border border-r-0 rounded-l-xl bg-gray-100">
                                        +569
                                    </span>
    ...
    <!-- SCRIPT PARA COUNTER DE CARACTERES -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Contador para modal crear
            const textareaCreate = document.querySelector(
                '#modalCrear textarea[name="perfeccionamiento_descripcion"]');
            const charCountCreate = document.getElementById('charCount');
            if (textareaCreate) {
                textareaCreate.addEventListener('input', function() {
                    charCountCreate.textContent = this.value.length + '/500';
                });
            }

            // Contadores para modales editar
            document.querySelectorAll('textarea[name="perfeccionamiento_descripcion"]').forEach((textarea) => {
                const itemId = textarea.closest('form')?.parentElement?.id?.replace('editar', '') || '';
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!checkbox || !fechaTermino) return;
