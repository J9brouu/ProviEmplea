<x-talento-layout>
    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-2xl shadow p-6 border-t-4 {{ $borderCv }}">
                ...
                <form method="POST" action="{{ route('talento.documentos.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="flex gap-3">
                        <input type="file" name="archivo" accept=".pdf,.doc,.docx"
                            class="flex-1 text-sm text-gray-500 file:mr-3 file:py-2 file:px-4 file:rounded-xl file:border-0 file:bg-blue-50 file:text-blue-700 file:font-medium hover:file:bg-blue-100 border border-gray-200 rounded-xl px-3 py-2">
                        <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-medium transition text-sm">
                            {{ $cv ? 'Reemplazar' : 'Subir' }}
                        </button>
                    </div>
                </form>
                @if($cv)
                    <form method="POST" action="{{ route('talento.documentos.destroy', $cv->id) }}" class="mt-3" id="form-cv-del">
                        @csrf @method('DELETE')
                        <button type="button"
            ...
            <div class="bg-white rounded-2xl shadow p-6 border-t-4 {{ $borderRes }}">
                <div class="flex items-start justify-between mb-4">
                    <div>
                ...
                @if($residencia)
                    <form method="POST" action="{{ route('talento.documentos.destroy', $residencia->id) }}" class="mt-3" id="form-res-del">
                        @csrf @method('DELETE')
                        <button type="button"
            ...
            {{-- ====== LEY 21.015 ====== --}}
            @php $borderDisc = $certDisc ? ($certDisc->estado === 'aprobado' ? 'border-blue-500' : ($certDisc->estado === 'rechazado' ? 'border-red-500' : 'border-yellow-400')) : 'border-gray-200'; @endphp
            <div class="bg-white rounded-2xl shadow p-6 border-t-4 {{ $borderDisc }}">
                <div class="flex items-start justify-between mb-4">
                    <div>
                    ...
                    @if($certDisc)
                        @if($certDisc->estado === 'aprobado')
                            <span class="bg-blue-100 text-blue-700 text-xs px-3 py-1 rounded-full font-semibold">✓ Verificado</span>
                        ...
                        @endif
                    @else
                        <span class="bg-gray-100 text-gray-500 text-xs px-3 py-1 rounded-full font-semibold">Opcional</span>
                    @endif
                </div>
                ...
                @if($certDisc)
                    ...
                    @endif
                    @if($certDisc->estado === 'aprobado')
                        <div class="bg-blue-50 border border-blue-200 text-blue-700 text-sm rounded-xl px-4 py-3 mb-4">
                ...
                @if($certDisc)
                    <form method="POST" action="{{ route('talento.documentos.destroy', $certDisc->id) }}" class="mt-3" id="form-disc-del">
                        @csrf @method('DELETE')
                        <button type="button"
